...
<script>
// === START EDIT: Improved Add Member Logic with Error Handling ===
...
document.getElementById('addMemberForm').addEventListener('submit', function(e) {
    ...
    submitBtn.innerHTML = `<span class="spinner-border spinner-border-sm"></span> ກำลังบันทึก...`;
    ...
    .then(res => {
        if (!res.ok) {
            // Handles server errors like 500, 404 etc.
    ...
    .catch(error => {
        // Catches network errors or if the response is not valid JSON (due to a PHP error)
        console.error('Fetch Error:', error);
        alert('ເກີດຂໍ້ຜິດພາດຮ້າຍແຮງ! ກະລຸນາກວດສອບ Console Log ໃນ Browser ເພື່ອເບິ່ງລາຍລະອຽດ.');
    })
    .finally(() => {
        // This will run whether the fetch succeeded or failed
        submitBtn.disabled = false;
        submitBtn.innerHTML = 'ບັນທຶກ';
    });
});
// === END EDIT ===

// --- Adjust Balance Logic (remains the same) ---
